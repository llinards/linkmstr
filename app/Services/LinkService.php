...
class LinkService
{
    ...
    public function createLink(array $data): Link
    {
        ...
        // Handle custom_code - generate unique code if custom_code is null, empty string, or whitespace
        $link->short_code = ! empty(trim($data['custom_code'] ?? ''))
            ? $data['custom_code']
            : Link::generateUniqueShortCode();
